...
def split_clauses(text: str) -> List[str]:
    """Split text into clause-like segments for compensation parsing.

    We split on:
      - sentence terminators (periods/semicolons)
      - bullet markers (• and similar)
      - list enumerators like (i), (ii), (iii) when they appear to introduce list items

    This is intentionally conservative (we don't try to build perfect sentences) but it
    materially reduces cross-contamination where adjacent bullets mention different pay
    concepts (e.g., "No annual cash incentive" immediately followed by an equity award).
    """
    if not text:
        return []
    ...
    # Normalize common bullet glyphs into hard breaks.
    # Keep the enumerator tokens (e.g., "(i)") in place so downstream logic can use them.
    t = re.sub(r"\s*[\u2022•·‣◦]\s*", ". ", t)

    # Insert breaks before roman-numeral list items (i), (ii) ... when they look like list delimiters.
    # Example patterns in 8-Ks: ": (i) ...; (ii) ..." or "will receive (i) ...".
    t = re.sub(
        r"\s+(?=\(\s*(?:i|ii|iii|iv|v|vi|vii|viii|ix|x|xi|xii|xiii|xiv|xv)\s*\)\s*)",
        ". ",
        t,
        flags=re.IGNORECASE,
    )

    # Protect abbreviations, split, then unprotect.
    t2 = _protect_abbrev_periods(t)
...
# -----------------------------
# Compensation extraction constants
# -----------------------------
# NOTE: These patterns/keyword sets are used by extract_compensation(). They were accidentally
# omitted in v8.2, which caused runtime NameErrors (e.g., BASE_SALARY_PATTERNS not defined).

# Match $ amounts with optional scale words (million/billion/etc.).
DOLLAR_RE = (
    r"(?:US\$|\$)\s*"
    r"(?:\d{1,3}(?:,\d{3})+|\d+)(?:\.\d+)?"
...
def normalize_position_title(raw_title: str, position_query: str, is_interim: bool = False) -> str:
    ...
    def _strip_leading(s: str) -> str:
        ...
        s2 = re.sub(r"^to\s+(?:the\s+)?(?:role|position)s?\s+of\s+", "", s2, flags=re.IGNORECASE)
        return s2.strip()
...
def _compile_exec_regexes(position_titles: List[str]) -> List[re.Pattern]:
    """
    Build regexes that try to catch common appointment language in 8-Ks (typically Item 5.02).
    ...
    Note: we keep these patterns intentionally "appointment-shaped" (verbs + person + role)
    to avoid false positives from generic mentions ("reporting to the CEO", "advisor to the CEO", etc.).
    Downstream filters in `detect_exec_matches()` further exclude common false-positive constructs.
    """
    ...
    # 8-Ks frequently insert appositives between the executive name and the role, e.g.:
    #   "Jane Doe (49) as CEO" or "Jane Doe (age 49), our COO, has been elected CEO"
    # so we allow a short parenthetical + comma appositive after the name.
    #
    # Also allow multiple names in a single clause (e.g., "A and B were promoted to the roles of CEO...").
    name_list = rf"{NAME_RE}(?:\s*(?:,|\band\b)\s*{NAME_RE})*"
    paren_app = rf"(?:\s*\([^)]{{0,60}}\))?"                 # (49), (age 58), etc.
    comma_app = rf"(?:\s*,[^.;\n]{{0,90}}?,)?"                # ", age 58," / ", currently COO,"
    name_app = rf"(?P<name>{name_list}){paren_app}{comma_app}"

    # "other person" in "to succeed/replace ..." clauses is often abbreviated ("Mr. Zaffino")
    # so we allow either a full name or an honorific + last name.
    other_short = r"(?:Mr\.|Ms\.|Mrs\.|Dr\.)\s+(?-i:[A-Z][A-Za-z\.\-’\']+)"
    other_person = rf"(?:{NAME_RE}|{other_short})"
    other_name_app = rf"(?:{other_person})(?:\s*\([^)]{{0,60}}\))?(?:\s*,[^.;\n]{{0,90}}?,)?"

    patterns = [
        # "Jane Doe will succeed John Roe as Chief Financial Officer"
        rf"{name_app}\s+will\s+(?:succeed|replace)\s+{other_name_app}\s+as\s+(?:the\s+)?(?:its\s+)?{title_re}",

        # "appointed John Doe to succeed Mr. Roe as Chief Financial Officer"
        rf"\b(?P<lemma>{verbs_alt})\b\s+{name_app}\s+to\s+(?:succeed|replace)\s+{other_name_app}\s+as\s+(?:the\s+)?(?:its\s+)?{title_re}",

        # "appointed John Doe as Chief Financial Officer"
        rf"\b(?P<lemma>{verbs_alt})\b\s+{name_app}\s+(?:as|to\s+serve\s+as|to\s+be)\s+(?:the\s+)?(?:its\s+)?{title_re}",

        # "appointed John Doe chief executive officer" (no 'as')
        rf"\b(?P<lemma>{verbs_alt})\b\s+{name_app}\s+(?:the\s+)?(?:its\s+)?{title_re}",

        # "John Doe was appointed Chief Financial Officer" (optional 'as')
        rf"{name_app}\s+(?:has\s+been|was|is|were|are)\s+\b(?P<lemma>{verbs_alt})\b\s+(?:(?:as|to\s+serve\s+as|to\s+be)\s+)?(?:the\s+)?(?:its\s+)?{title_re}",

        # "A and B were promoted to the roles of Chief Executive Officer ..."
        rf"{name_app}\s+(?:has\s+been|was|is|were|are)\s+\b(?P<lemma>{verbs_alt})\b\s+to\s+(?:the\s+)?(?:role|position)s?\s+of\s+(?:the\s+)?(?:its\s+)?{title_re}",

        # "the appointment of John Doe to serve as Chief Financial Officer"
        rf"\b(?:the|an|this)\s+appointment\s+of\s+{name_app}\s+(?:as|to\s+serve\s+as|to\s+be)\s+(?:the\s+)?(?:its\s+)?{title_re}",

        # "John Doe will be appointed Co-Chief Executive Officer"
        rf"{name_app}\s+will\s+be\s+\b(?P<lemma>{verbs_alt})\b\s+(?:(?:as|to\s+serve\s+as|to\s+be)\s+)?(?:the\s+)?(?:its\s+)?{title_re}",

        # "John Doe will join / will be joining the Company as President & CEO"
        rf"{name_app}\s+will\s+(?:join|be\s+joining)\b[^.;\n]{{0,140}}?\s+as\s+(?:the\s+)?(?:its\s+)?{title_re}",

        # "John Doe will become Chief Executive Officer"
        rf"{name_app}\s+will\s+becom(?:e|ing)\s+(?:the\s+)?(?:its\s+)?{title_re}",
        ...
        rf"{name_app}\s+will\s+assum(?:e|ing)\s+(?:the\s+)?(?:role\s+of\s+)?{title_re}",
    ]
...
def detect_exec_matches(
    ...
    # Helpers
    def _trim_reporting_phrases(t: str) -> str:
        if not t:
            return t
    ...
    def _title_hits_position(t: str) -> bool:
        ...
        for tok in (pos_titles or []):
            ...
            if not tok:
                continue
            if len(tok) <= 4:
                if re.search(rf"\b{re.escape(tok)}\b", t, re.IGNORECASE):
                    return True
            else:
    ...
    def _extract_names_from_blob(blob: str) -> List[str]:
        """
        Extract one or more executive names from a regex-captured blob.

        Some filings list multiple appointees in a single clause (e.g., "A and B were promoted...").
        Our regexes intentionally allow that, but downstream logic expects one ExecMatch per person.
        """
        if not blob:
            return []
        out: List[str] = []
        for nm in re.finditer(rf"(?=({NAME_RE}))", blob):
            cand = norm_ws(nm.group(1))
        ...
        # Fallback: if regex didn't find sub-names, but the blob itself looks like a person, keep it.
        if not deduped and _looks_like_person_name(blob):
            return [blob]
        return deduped

    for rx in regexes:
        for m in rx.finditer(item_text):
            name_blob = norm_ws(m.group("name"))
            cand_names = _extract_names_from_blob(name_blob)
            if not cand_names:
                continue
            ...
            # effective date: first one in nearby context
            eff = None
...
def _infer_award_types_for_clause(clause: str, money_matches: List[re.Match]) -> List[str]:
    ...
    for mm in money_matches:
        local = clause[max(0, mm.start() - 220) : min(len(clause), mm.end() + 220)].lower()
...
def _equity_bucket_for_context(clause_low: str, local_low: str) -> str:
    ...
    one_time_strong_kws = ("one-time", "one time", "sign-on", "sign on", "signing", "make-whole", "make whole", "inducement", "replacement", "makewhole", "new hire", "new-hire")
...
def extract_compensation(text: str) -> ExtractedComp:
    ...
    Key behaviors:
      - Classifies each $ amount using LOCAL context (reduces false cash/equity attribution).
      - Preserves distinct awards that share the same $ value (e.g., RSU + PSU with same target).
      - Avoids double-counting aggregates when an aggregate + components are both disclosed in the same clause.
      - Excludes conditional subset amounts (e.g., "$13 million, $10 million of which is subject to repayment").
      - Tracks section-level equity context (e.g., "New Hire Equity Awards" vs "Long-Term Incentive Equity Awards"),
        which is critical when award clauses only use valuation phrases ("grant date value") without explicit one-time/annual words.
    """
    comp = ExtractedComp()
    if not text:
        return comp

    # Filter out tiny dollar amounts (par value, nominal fees) that frequently appear in 8-K cover pages
    # and can pollute totals.
    MIN_COMP_AMOUNT_USD = 10_000
    ...
    # Fallback: pick the best "% of base salary" that looks like a target bonus (handles long parenthetical pro-rating).
    if best_pct_m is None:
        pct_of_base_re = re.compile(
            r"(?P<pct>\d{1,3}(?:\.\d+)?)\s*%\s+of\s+[^\n]{0,90}?\b(?:annual\s+)?base\s+salary\b",
            re.IGNORECASE,
        )
        best_score = -1e9
        for m in pct_of_base_re.finditer(text):
            pct = m.group("pct")
            try:
                pct_v = float(pct)
            except Exception:
                continue
            # context window BEFORE the percent is the most informative
            w0 = max(0, m.start() - 450)
            w1 = min(len(text), m.start() + 80)
            ctx_low = text[w0:w1].lower()

            score = 0.0
            if any(k in ctx_low for k in ("bonus", "incentive", "sti", "short term", "short-term", "annual performance bonus")):
                score += 2.0
            if "target" in ctx_low or "targeted" in ctx_low:
                score += 1.0
            if any(k in ctx_low for k in ("maximum", "max", "cap")):
                score -= 0.5
            # Prefer plausible executive target ranges (but don't hard-filter).
            if 50.0 <= pct_v <= 400.0:
                score += 0.5
    ...
    if best_pct_m is not None:
        pct = best_pct_m.group("pct")
        try:
            comp.target_bonus_pct = float(pct)
            comp.target_bonus = f"{pct}%"
        except Exception:
            pass
        comp.evidence_snippets.append(sentence_snippet(text, best_pct_m.start(), best_pct_m.end()))
    ...
    # Section-level equity context (persist across adjacent clauses / bullets)
    equity_mode: Optional[str] = None   # "annual" | "one_time" | None
    no_annual_equity: bool = False
    ...
    for clause in clauses:
        ...
        # Update equity section context
        if re.search(r"\bno\s+(?:additional\s+)?annual\s+equity\s+awards?\b", clause_low):
            no_annual_equity = True

        if "exclusive long-term incentive" in clause_low and ("during the term" in clause_low or "employment term" in clause_low):
            equity_mode = "one_time"
            no_annual_equity = True

        if re.search(r"\bnew\s*(?:-|\s)?hire\s+equity\s+awards?\b", clause_low):
            equity_mode = "one_time"

        # Some filings separate the heading ("New Hire Equity Awards") from the award bullets.
        # If we see "new hire" alongside equity language, treat subsequent awards as one-time.
        if re.search(r"\bnew\s*(?:-|\s)?hire\b", clause_low) and _contains_any(clause_low, EQUITY_KWS):
            equity_mode = "one_time"

        if "inducement" in clause_low and ("award" in clause_low or "grant" in clause_low):
            equity_mode = "one_time"
        ...
        # If a clause explicitly discusses "annual long-term incentive ... target value", treat as annual.
        if re.search(r"\bannual\s+long\s*(?:-|\s)?term\s+incentive\b", clause_low) or re.search(r"\bannual\s+long\s*(?:-|\s)?term\s+incentive\s+equity\s+award\b", clause_low):
            equity_mode = "annual"
        ...
        for i, mm in enumerate(money_matches):
            ...
            if amt_usd is not None and amt_usd < MIN_COMP_AMOUNT_USD:
                continue
            ...
            # Context windows
            local_wide = clause[max(0, mm.start() - 220) : min(len(clause), mm.end() + 220)]
            local_wide_low = local_wide.lower()

            local_tight = clause[max(0, mm.start() - 70) : min(len(clause), mm.end() + 70)]
            local_tight_low = local_tight.lower()
            ...
            equity_near = _contains_any(local_wide_low, EQUITY_KWS)
            # cash keywords should be tight to avoid cross-contamination
            cash_near = _contains_any(local_tight_low, CASH_KWS)
            one_time_near = _contains_any(local_wide_low, ONE_TIME_KWS)

            cash_strong_near = bool(
                re.search(r"\bcash\s+(?:payment|bonus|award|retention|incentive)\b", local_tight_low)
            )

            award_type = award_types[i] if i < len(award_types) else _infer_award_type(local_wide_low)

            mentions.append(
                {
                    "amt_str": amt_str,
                    "amt_usd": amt_usd,
                    "local_low": local_wide_low,
...
def compensation_brief(comp: "ExtractedComp") -> str:
    """
    Human-readable, compact comp summary for UI/CSV.
    This is intentionally heuristic and should be treated as a quick scan aid.
    """
    if comp is None:
    ...
    if comp.target_bonus:
        # target_bonus can be a % string or a $ amount string, depending on what matched
        if comp.target_bonus_pct is not None:
            bits.append(f"Target cash incentive {comp.target_bonus} ({float(comp.target_bonus_pct):g}%)")
        else:
            bits.append(f"Target cash incentive {comp.target_bonus}")
    ...
    elif comp.equity_target_annual_usd_total is not None:
        bits.append(f"Annual/target equity ${comp.equity_target_annual_usd_total:,}")

    if comp.equity_annual_pregrant_values:
        bits.append("Annual equity (pre-grant) " + ", ".join(comp.equity_annual_pregrant_values))
    elif comp.equity_annual_pregrant_usd_total is not None:
        bits.append(f"Annual equity (pre-grant) ${comp.equity_annual_pregrant_usd_total:,}")

    if comp.equity_annual_advance_values:
        bits.append("Annual equity (advance) " + ", ".join(comp.equity_annual_advance_values))
    elif comp.equity_annual_advance_usd_total is not None:
        bits.append(f"Annual equity (advance) ${comp.equity_annual_advance_usd_total:,}")
