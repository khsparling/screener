...
class ExtractedComp:
    ...
    # Structured-ish counts for equity awards when the filing discloses shares/units instead of $ value
    # Examples: '50,000 RSUs', '120,000 stock options'
...
MONEY_PARSE_RE = re.compile(
    ...
    r"(?:\s*(?P<scale>million|billion|thousand|bn|m|b|k)\b)?",
    re.IGNORECASE,
)
...
HONORIFIC_PREFIX_RE = re.compile(r"^(?:Mr\.|Ms\.|Mrs\.|Dr\.)\s+", re.IGNORECASE)

# Words that commonly appear in non-person "names" when regexing SEC text
NON_PERSON_NAME_TOKENS = {
    "company", "registrant", "issuer",
    "board", "committee", "compensation", "nominating", "governance",
    "named", "executive", "officer", "officers", "employee", "employees", "eligible",
    "director", "directors",
    "plan", "tier",
}

def normalize_person_name(raw: str) -> str:
    """Normalize captured names; also collapse obvious repeats like 'Ms. X. Ms. X'."""
    n = norm_ws(raw or "")

    # Collapse exact repeats like 'Ms. Buese. Ms. Buese' or 'John Doe; John Doe'
    m = re.match(r"^\s*(?P<x>.+?)\s*[.;]\s*(?P=x)\s*$", n, flags=re.IGNORECASE)
    if m:
        n = m.group("x")

    # Strip common HTML->text lead-ins that can leak into the capture group (e.g., 'Board. Jane Doe')
    n = re.sub(r"^(?:the\s+)?(?:company|board|committee)\b\s*[.:;,-]\s+", "", n, flags=re.IGNORECASE)

    return n.strip(" ,;:()[]")

def strip_honorific(n: str) -> str:
    return HONORIFIC_PREFIX_RE.sub("", (n or "").strip())

def looks_like_person_name(name: str) -> bool:
    """Heuristic person-name validator to reject phrases like 'Company’s Named Executive Officers'."""
    n = normalize_person_name(name)
    if not n:
        return False

    core = strip_honorific(n)
    toks = [t.strip(".\"'’").lower() for t in re.findall(r"[A-Za-z][A-Za-z\-\.'’]*", core)]
    if not toks:
        return False
    if any(t in NON_PERSON_NAME_TOKENS for t in toks):
        return False

    # Allow honorific + surname only (common in 8-K prose)
    if HONORIFIC_PREFIX_RE.match(n) and len(toks) >= 1:
        return True

    # Typical person names are 2-5 tokens (first/last, optional middle)
    return 2 <= len(toks) <= 5

def title_is_relationship_to_role(title: str, pos_titles: List[str]) -> bool:
    """True if the matched 'title' looks like a relationship to the role (advisor/reporting), not the role itself."""
    tl = (title or "").lower()
    if not tl:
        return False

    # Build a target-token regex from the position synonyms (case-insensitive)
    # Example: CEO -> 'chief executive officer|ceo|principal executive officer|...'
    toks = [t.strip() for t in (pos_titles or []) if t and t.strip()]
    if not toks:
        return False
    target_alt = "|".join(re.escape(t.lower()) for t in toks)
    target_re = rf"(?:{target_alt})"

    rel_patterns = [
        rf"\b(advisor|adviser)\b[^.;]{{0,80}}\bto\b[^.;]{{0,80}}\b{target_re}\b",
        rf"\b(reporting|reports?|report)\b[^.;]{{0,25}}\bto\b[^.;]{{0,80}}\b{target_re}\b",
        rf"\b(executive\s+assistant|assistant)\b[^.;]{{0,80}}\bto\b[^.;]{{0,80}}\b{target_re}\b",
        rf"\boffice\b[^.;]{{0,25}}\bof\b[^.;]{{0,80}}\b{target_re}\b",
        rf"\b(tier|eligible\s+employee)\b[^.;]{{0,160}}\b{target_re}\b",
        rf"\bfor\b\s+(?:our|the)\s+\b{target_re}\b",  # 'for our CEO' (plan eligibility tiers)
    ]
    return any(re.search(p, tl, flags=re.IGNORECASE) for p in rel_patterns)

def title_looks_like_real_officer_title(title: str, pos_titles: List[str]) -> bool:
    """Reject common false-positive title shapes for CEO/CFO/etc searches.

    The matched 'title' should look like an actual officer title, not a sentence fragment
    that merely *mentions* the target role (e.g., '... reporting to the CEO', or a plan tier).
    """
    t = norm_ws(title or "")
    if not t:
        return False
    ...
    # Never part of an officer title; usually compensation-plan language
    noise = (
        "eligible employee",
        "tier",
        "performance period",
    ...
    if any(k in tl for k in noise):
        return False

    # Relationship titles (advisor/reporting) are not the target role
    if title_is_relationship_to_role(t, pos_titles):
        return False

    # If our regex title window bleeds across prose (often due to missing punctuation in HTML->text),
    # we can end up with fragments like:
    #   'Chief Operating Officer On February 3, 2025, the Company announced ... the CEO'
    # If the *first* appearance of the target token is after a month/year mention, treat as a likely false positive.
    if pos_titles:
        positions: List[int] = []
        for tok in pos_titles:
            tok = (tok or "").strip()
            if not tok:
                continue
            p = tl.find(tok.lower())
        ...
        if positions:
            ...
            m_year = re.search(r"\b(?:19|20)\d{2}\b", tl)
            if m_year and target_pos > m_year.start():
                return False
            m_month = re.search(
                r"\b(january|february|march|april|may|june|july|august|september|october|november|december)\b",
                tl,
            )
            if m_month and target_pos > m_month.start():
                return False
...
def detect_exec_matches(text: str, position_query: str) -> Tuple[bool, Dict[str, Any], List[ExecMatch]]:
    ...
    # Post-filters to reduce common false positives:
    #  - relationship titles like 'advisor/reporting to the CEO'
    #  - compensation-plan language like 'Tier 1 (for our CEO)'
    #  - non-person 'names' like 'Company’s Named Executive Officers'
    filtered: List[ExecMatch] = []
    for mm in deduped:
        mm2 = dataclasses.replace(mm, name=normalize_person_name(mm.name), title=norm_ws(mm.title))
        # Upgrade event_type if we have clear appointment language
        if (mm2.event_type or "unknown") == "unknown":
            if re.search(r"\b(appoint(?:ed|s)?|name(?:d|s)?|elect(?:ed|s)?|designat(?:ed|es)?|select(?:ed|s)?)\b", mm2.context, re.IGNORECASE):
                mm2.event_type = "appointment"
        if not looks_like_person_name(mm2.name):
            continue
        if not title_looks_like_real_officer_title(mm2.title, pos_titles):
            continue
        filtered.append(mm2)
...
def _classify_equity_snippet(s: str) -> str:
    ...
    Classify an equity-related sentence into:
      ...
      - annual_target: annual/ongoing or target equity/LTI opportunity
      - other: not equity

    Notes:
      * Many filings describe both one-time "inducement/make-whole" awards and ongoing/annual LTI.
      * This is intentionally heuristic and uses a small scoring model to reduce misclassification.
    """
    s_low = (s or "").lower()
    ...
    if "grant date value" in s_low:
        # Often used for sign-on / inducement awards, but can appear for annual grants too.
        one_time_score += 1

    # Strong annual/ongoing signals
    if any(k in s_low for k in ANNUAL_TARGET_KWS):
        annual_score += 4
    if re.search(r"\b(annual|ongoing|each year|per year)\b", s_low):
        annual_score += 2

    # Tie-breakers / weak signals
    if "target" in s_low:
        annual_score += 1
    if re.search(r"\b(one-?time|sign(?:ing)?|inducement|make-?whole|replacement|initial)\b", s_low):
        one_time_score += 1
    ...
    # Default: if equity is mentioned but unclear, treat as annual/target
    return "annual_target"
...
def extract_compensation(text: str) -> ExtractedComp:
        ...
        for m in pat.finditer(text):
            ...
            if field == "base_salary" and comp.base_salary is None:
                ...
                if money:
                    ...
                    # Try to capture share/unit counts (RSUs/options) when the filing doesn't disclose a $ value.
                    g1 = (m.group(1) or "").strip()
                    g2 = (m.group(2) or "").strip()
                    if g2 and re.fullmatch(r"\d{1,3}(?:,\d{3})*", g2):
                        lbl_low = g1.lower()
                        if "rsu" in lbl_low or "restricted stock" in lbl_low:
                            entry = f"RSUs {g2} units"
                        elif "option" in lbl_low:
                            entry = f"Options {g2} shares"
                        else:
                            entry = f"{g1} {g2}".strip()

                        cat = _classify_equity_snippet(eq_snip)
                        if cat == "one_time":
                            comp.equity_one_time_counts.append(entry)
                        else:
                            comp.equity_target_annual_counts.append(entry)
    ...
    comp.equity_target_annual_counts = _dedupe(comp.equity_target_annual_counts)
    comp.equity_one_time_counts = _dedupe(comp.equity_one_time_counts)
...
def compensation_brief(comp: ExtractedComp) -> str:
    ...
    if comp.equity_target_annual_counts:
        bits.append("Equity target/annual counts " + ", ".join(comp.equity_target_annual_counts))
    if comp.equity_one_time_counts:
        bits.append("One-time equity counts " + ", ".join(comp.equity_one_time_counts))
...
def process_filing(
    ...
    # Candidate exhibits likely to contain comp terms (employment/offer letters, severance, press releases)
    ...
    # Fetch (limited) document texts and keep them separate so we can do *person-scoped* compensation extraction.
    doc_blobs: List[Tuple[str, str, str, str]] = []  # (label, doc_type, url, text)
    doc_blobs.append((filing2.primary_doc or "primary", filing2.form, filing2.primary_url(), primary_text))
    ...
    for d in likely_docs[:max_exhibits]:
        try:
            ...
            continue
        if txt:
            doc_blobs.append((d.filename or "exhibit", d.doc_type or "", d.url, txt))

    combined_text = "\n\n".join([t for (_, _, _, t) in doc_blobs if t])
    ...
    def _name_patterns(name: str) -> List[re.Pattern]:
        ...
        if core and len(core) <= 80:
            pats.append(re.compile(rf"\b{re.escape(core)}\b", flags=re.IGNORECASE))

        if toks:
            last = toks[-1]
            # Last-name-only matching can be noisy; prefer honorific + last name.
            if len(last) >= 4:
                pats.append(re.compile(rf"\b(?:Mr|Ms|Mrs|Dr)\.\s+{re.escape(last)}\b", flags=re.IGNORECASE))

        # Dedupe patterns
        seen_p: Set[str] = set()
        out: List[re.Pattern] = []
        for p in pats:
            if p.pattern in seen_p:
                continue
            seen_p.add(p.pattern)
            out.append(p)
        return out

    def _windows_around(text: str, pats: List[re.Pattern], window: int = 2200, max_windows: int = 6) -> List[str]:
    ...
    def _focus_text_for_match(mm: ExecMatch) -> str:
        parts: List[str] = []
        if mm.context:
            parts.append(mm.context)

        pats = _name_patterns(mm.name or "")
        for (label, doc_type, url, txt) in doc_blobs:
            if not txt:
                continue
            if pats and not any(p.search(txt) for p in pats):
                continue

            # Employment/offer/severance agreements: include more context (often structured)
            if (doc_type or "").upper().startswith("EX-10"):
                parts.append(txt[:80_000])
            else:
                parts.extend(_windows_around(txt, pats, window=2200, max_windows=6))

        # Safety net: if we didn't pick up much, include a limited slice of Item 5.02 from the primary doc
        if sum(len(p) for p in parts) < 1200 and primary_text:
            parts.append(slice_item_502(primary_text)[:12_000])
    ...
    for mm in matches[:3]:  # cap for sanity
        focus_text = _focus_text_for_match(mm)
                ...
                w.writerow(
                    {
                        ...
                        "equity_target_annual_counts": "; ".join(comp.equity_target_annual_counts),
                        "equity_one_time_counts": "; ".join(comp.equity_one_time_counts),
